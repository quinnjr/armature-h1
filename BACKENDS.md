...
| A handler that drops the request `Body` **unread** before a `101` response | The upgrade is forfeited and the connection closes: the unread bytes are still on the wire and would reach the consumer as `Upgraded::buffered`, which is documented as the peer's first post-upgrade frames | Identical — the bridge gates the handoff on the same "body reached its end" signal (`bridge.rs`, `fully_read`) | **No divergence.** Listed here only because the neighbouring *retained*-body row does diverge, and the two are easy to conflate: the unread-body rule is enforced on both backends, the retained-body rule on native alone. |
